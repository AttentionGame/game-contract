...
interface IERC721Hexagon is IERC721 {
    struct Hexagon {
        string q;
        string r;
        string s;
    ...
    event CreateHexagon(address indexed owner, string indexed hexagonKey, Hexagon hexagon);
    ...
    function distance(string memory q, string memory r, string memory s) external pure returns (uint256);
    function isLinked(string memory q, string memory r, string memory s) external returns (bool);
}